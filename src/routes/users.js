...
router.post('/', async (req, res) => {
  try {
    const { role, name, email, password, agent_id, referral_code } = req.body;
    ...
    // If referral_code is provided, find the agent
    let finalAgentId = agent_id;
    if (referral_code) {
      const agent = await User.findOne({ referral_code, role: 'agent' });
      if (agent) {
...
router.get('/my-referrals', verifyToken, async (req, res) => {
  try {
    ...
    const [referredUsers, total] = await Promise.all([
      User.find({ agent_id: req.user._id, is_active: true })
        .select('name email createdAt is_active')
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(parseInt(limit)),
      User.countDocuments({ agent_id: req.user._id, is_active: true })
    ]);
...
router.get('/agent-stats', verifyToken, async (req, res) => {
  try {
    ...
    // Get reports generated by the agent
    const agentReportsCount = await Report.countDocuments({
    ...
    res.json({
      ...
      data: {
        ...
        my_reports: agentReportsCount,
        total_earned: totalEarned,
        available_balance: commissionBalance - pendingWithdrawalAmount,
        pending_withdrawal: pendingWithdrawalAmount,
        total_withdrawn: paidAmount,
        wallet_balance: commissionBalance
      }
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
